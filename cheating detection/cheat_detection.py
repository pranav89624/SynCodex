...
# Constants for eye and iris landmarks
LEFT_EYE = [33, 133]
RIGHT_EYE = [362, 263]
LEFT_IRIS = [474]
RIGHT_IRIS = [469]

# Function to calculate eye position ratio
...
# Function to run cheat detection
def run_detection():
    cap = cv2.VideoCapture(0)
    warnings = 0
    max_warnings = 3
    last_warning_time = 0
    cooldown = 3  # Seconds to wait before issuing another warning
    ...
    with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face_detection, \
        ...
        while cap.isOpened():
            ...
            # Process the frame for face detection and face landmarks
            ...
            # Handle no face or multiple faces detection
            if num_faces == 0:
            ...
            # Handle face landmarks for eye movement and iris detection
            if mesh_result.multi_face_landmarks:
                for face_landmarks in mesh_result.multi_face_landmarks:
                    ...
                    try:
                        # Extract eye corners and iris positions
            ...
            # Issue warning if cheat detected and handle cooldown
            ...
            # If max warnings reached, terminate session
            if warnings >= max_warnings:
                cv2.putText(frame, "Cheating Detected! Session Terminated.", (30, 150),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                cv2.imshow("Cheating Detection", frame)
                cv2.waitKey(3000)  # Display the message for 3 seconds before terminating
                break
